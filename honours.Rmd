...
female_treatment1_plot <- pruned_eEPSC_df$all_cells %>%
  ...
  labs(x = "Time (min)", y = "eEPSC Amplitude (% Baseline)") +
  geom_hline(yintercept = 100, linetype = "dashed") +
  scale_colour_manual(values = c("#f5e0b3", "#f19cac"), labels = c("Naive", "Acute")) +
  patchclampplotteR_theme()

# want to add t-test
# want to add n = in the bottom corner
...
# the boxes
  geom_boxplot(aes(fill = interaction(category, treatment), color = interaction(category, treatment),fatten = NULL), 
               alpha = 0.6, 
               color = "black",
