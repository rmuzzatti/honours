...
# long format data
acute_female_AP_data_plot_df <- data.frame(
  letter = rep(acute_females, each = 2), # Repeat letters for both Baseline and HFS
  state = rep(c("Baseline", "HFS"), times = length(acute_females)), # Create a column for the state
  peak_amplitude = c(acute_female_baseline_AP_data, acute_female_HFS_AP_data) # Combine Baseline and HFS data
)


# plot
acute_female_AP_data_plot <- acute_female_AP_data_plot_df %>%
...
# long format data
AM251_female_AP_data_plot_df <- data.frame(
  letter = rep(AM251_females, each = 2), # Repeat letters for both Baseline and HFS
  state = rep(c("Baseline", "HFS"), times = length(AM251_females)), # Create a column for the state
  peak_amplitude = c(AM251_female_baseline_AP_data, AM251_female_HFS_AP_data) # Combine Baseline and HFS data
)


# plot
AM251_female_AP_data_plot <- AM251_female_AP_data_plot_df %>%
  ggplot(aes(x = state, y = peak_amplitude, group = letter)) +
...
repeated_female_baseline_AP_data <- subset(repeated_female_AP_data, state == "Baseline")[, c("letter", "peak_amplitude")]
repeated_female_HFS_AP_data <- subset(repeated_female_AP_data, state == "HFS")[, c("letter", "peak_amplitude")]

# Merge the two subsets by 'letter' to keep the correct pairing
repeated_female_AP_data_plot_df <- merge(repeated_female_baseline_AP_data, 
                                         repeated_female_HFS_AP_data, 
                                         by = "letter", 
                                         suffixes = c("_baseline", "_hfs"))
...
repeated_female_baseline_AP_data <- dplyr::select(repeated_female_baseline_AP_data, letter, peak_amplitude)
repeated_female_HFS_AP_data <- dplyr::select(repeated_female_HFS_AP_data, letter, peak_amplitude)
...
# Reshape to long format (one row per letter and state)
repeated_female_AP_data_plot_df <- data.frame(
  letter = rep(repeated_female_AP_data_plot_df$letter, each = 2),
  state = rep(c("Baseline", "HFS"), times = nrow(repeated_female_AP_data_plot_df)),
  peak_amplitude = c(repeated_female_AP_data_plot_df$peak_amplitude_baseline,
                     repeated_female_AP_data_plot_df$peak_amplitude_hfs)
)


# plot
repeated_female_AP_data_plot <- repeated_female_AP_data_plot_df %>%
  ggplot(aes(x = state, y = peak_amplitude, group = letter)) +
...
## Repeated Measures ANOVA for naive
```{r}
# convert to long
...
# naive
naive_anova <- df_long %>% 
  filter(sex == "Female", category == 1, treatment == "Control") %>%
  aov(time_value ~ time, data = .)
...
naive_anova.resid <- resid(naive_anova)  
qqnorm(naive_anova.resid)
qqline(naive_anova.resid, col="red")
shapiro.test(naive_anova.resid)
...
# acute
acute_anova <- df_long %>% 
  filter(sex == "Female", category == 2, treatment == "Control") %>%
  aov(time_value ~ time, data = .)
...
acute_anova.resid <- resid(acute_anova)  
qqnorm(acute_anova.resid)
qqline(acute_anova.resid, col="red")
shapiro.test(acute_anova.resid)
hist(acute_anova.resid)
