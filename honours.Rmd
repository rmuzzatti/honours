...
```
### overall summary percent change mean and SE
```{r}
...
# x-axis subtitles  
  ...
  stat_compare_means(
    ...
    comparisons = list(c("1.Control", "2.Control"), c("1.Control", "2.AM251"), c("2.Control", "2.AM251")), # not sure if this is accurate...
    textsize = 5
) + 
...
scale_x_discrete(labels = c("Naive", "Acute", "Repeated")) +
  ...
  stat_compare_means(
    ...
    test.arg = list(paired = FALSE),
    label = "p.format", # p.signif does the stars, p.format shows the p-value
    comparisons = list(c("1.Control", "2.Control"), c("1.Control", "3.Control"), c("2.Control", "3.Control")), 
    textsize = 5
) + 
...
# add group column
PPR_df <- PPR_df %>%
  mutate(group = interaction(category, treatment, sep = ".", drop = TRUE))
```

### PPR mean and averages
```{r}
# mean and SE output
female_baseline_PPR_summary_df <- PPR_plot_df %>%
  ...
  summarise(
    ...
    .groups = "drop"  # to remove the grouping structure in the result
  )
female_baseline_PPR_summary_df

female_HFS_PPR_summary_df <- PPR_plot_df %>%
  filter(sex == "Female", state == "HFS") %>%
  group_by(facet_group) %>%  
  summarise(
    n = n(),  
    mean_PPR = mean(mean_PPR_cell, na.rm = TRUE), 
...
female_baseline_PPR_plot <- PPR_plot_df %>%
  ...
  ggplot(aes(x = cat_treat, y = mean_PPR_cell)) +
...
# ggsave(female_baseline_PPR_plot, path = here("Figures"), file = "female_baseline_PPR_plot.png", width = 7, height = 5, units = "in", dpi = 300)

```
```{r}
# data
female_baseline_control_PPR_plot <- PPR_plot_df %>%
  filter(sex == "Female" & 
           category %in% c(1,2, 3) & 
...
female_baseline_control_PPR_plot
...
```{r}
# making the acute df
acute_AP_freq_df <- AP_freq_df %>%
  filter(group == "acute")

# don't need to test for HoV because paired

# anova
acute_AP_freq_anova <- ezANOVA(
  data = acute_AP_freq_df,
  dv = AP_frequency,
  wid = letter,
  within = .(state, sweep),
  type = 3,
...
# add group variable
AP_amp_anova_data$group[AP_amp_anova_data$letter %in% acute_females] <- "acute"
AP_amp_anova_data$group[AP_amp_anova_data$letter %in% AM251_females] <- "AM251"
AP_amp_anova_data$group[AP_amp_anova_data$letter %in% repeated_females] <- "repeated"
...
```{r}
# one-way ANOVA
female_baseline_amp_anova <- aov(log_t0to5 ~ cat_treat, data = female_amp_df)
...
# Plot
female_baseline_amp_anova_plot <- ggplot(female_amp_df, aes(x = cat_treat, y = log_t0to5)) +
  ...
  # Axis labels
  labs(x = NULL, y = "log of Baseline eEPSC Amplitude (pA)") +
...
```{r}
# post hoc
pairwise.t.test(female_amp_df$log_t0to5, female_amp_df$cat_treat, "none", data = female_amp_df) # no alpha level correction applied
# I only only care about 4 (biologically meaningful): naive to acute, naive to AM251, naive to repeated
# you can run with alpha level of 0.05/3 = 0.015
```
...
## ANOVA for Baseline PPR
```{r}
# data frame
female_PPR_df <- PPR_plot_df %>%
  filter(sex == "Female" & category %in% c(1,2, 3) & treatment %in% c("Control","AM251")) %>% 
  mutate(cat_treat = factor(
    interaction(category, treatment),
    levels = c("1.Control", "2.Control", "2.AM251", "3.Control")
...
library(car)
leveneTest(mean_PPR_cell ~ cat_treat, data = female_PPR_df)
# variances are homogeneous
```
...
female_baseline_PPR_anova <- aov(mean_PPR_cell ~ cat_treat, data = female_PPR_df %>% filter(state == "Baseline"))
...
```{r}
# residuals
female_baseline_PPR_anova_residuals <- residuals(female_baseline_PPR_anova)

# check normality of residuals
shapiro.test(female_baseline_PPR_anova_residuals)

hist(female_baseline_PPR_anova_residuals, breaks = 10, main = "Residuals Histogram")

qqnorm(female_baseline_PPR_anova_residuals)
qqline(female_baseline_PPR_anova_residuals, col = "red")
...
df_baseline <- female_PPR_df %>% filter(state == "Baseline")

# Plot
female_baseline_PPR_anova_plot <- ggplot(df_baseline, aes(x = cat_treat, y = mean_PPR_cell)) +
  ...
  stat_compare_means(method = "anova", label.y = max(df_baseline$mean_PPR_cell) + 0.5) + 


  # Remove legend
  guides(fill = "none", color = "none")

female_baseline_PPR_anova_plot
```
```{r}
# post hoc

pairwise.t.test(female_PPR_df$mean_PPR_cell, female_PPR_df$cat_treat, "none", data = female_PPR_df %>% filter(state == "Baseline")) # no alpha level correction applied
# I only only care about 4 (biologically meaningful): naive to acute, naive to AM251, naive to repeated
# you can run with alpha level of 0.05/3 = 0.015
```


## ANOVA for post HFS PPR
```{r}
# using same data frame as earlier, already know variances are homogeneous

female_HFS_PPR_anova <- aov(mean_PPR_cell ~ cat_treat, data = female_PPR_df %>% filter(state == "HFS"))
...
```{r}
# residuals
female_HFS_PPR_anova_residuals <- residuals(female_HFS_PPR_anova)

# check normality of residuals
shapiro.test(female_HFS_PPR_anova_residuals)

hist(female_HFS_PPR_anova_residuals, breaks = 10, main = "Residuals Histogram")

qqnorm(female_HFS_PPR_anova_residuals)
qqline(female_HFS_PPR_anova_residuals, col = "red")
...
df_HFS <- female_PPR_df %>% filter(state == "HFS")

# Plot
female_HFS_PPR_anova_plot <- ggplot(df_HFS, aes(x = cat_treat, y = mean_PPR_cell)) +
  ...
  stat_compare_means(method = "anova", label.y = max(df_HFS$mean_PPR_cell) + 0.5) + 


  # Remove legend
  guides(fill = "none", color = "none")

female_HFS_PPR_anova_plot
```
```{r}
# post hoc

pairwise.t.test(female_PPR_df$mean_PPR_cell, female_PPR_df$cat_treat, "none", data = female_PPR_df %>% filter(state == "HFS")) # no alpha level correction applied
# I only only care about 4 (biologically meaningful): naive to acute, naive to AM251, naive to repeated
# you can run with alpha level of 0.05/3 = 0.015
...
```{r}
# convert to long
df_long <- summary_eEPSC_df$percent_change_data %>%
  pivot_longer(
...
# adding a column for group name
summary_anova_df <- df_long %>%
  mutate(group = case_when(
...
### naive females
```{r}
# making a subset for just naive females
naive_summary_df <- summary_anova_df %>%
  filter(group == "naive")
# both time (interval) and letter are as factor which is good


# HoV
leveneTest(time_value ~ time, data = naive_summary_df)
# good
```
```{r}
# running anova

naive_summary_anova <- ezANOVA(
  data = naive_summary_df,
  dv = time_value,
  wid = letter,
  within = time,
  # between = group,
...
print(naive_summary_anova)
# ignore warning about letter, it is already a factor
```

```{r}
# normality of residuals

naive_summary_anova_lm_model <- lm(time_value ~ time, data = naive_summary_df)
naive_summary_anova_resid_vals <- residuals(naive_summary_anova_lm_model)

qqnorm(naive_summary_anova_resid_vals)
qqline(naive_summary_anova_resid_vals)
shapiro.test(naive_summary_anova_resid_vals)
# p is 0.01571 but its fine visually
```

### acute females
```{r}
# making a subset for just acute females
acute_summary_df <- summary_anova_df %>%
  filter(group == "acute")
# both time (interval) and letter are as factor which is good


# HoV
leveneTest(time_value ~ time, data = acute_summary_df)
# good
```
...
acute_summary_anova <- ezANOVA(
  data = acute_summary_df,
  dv = time_value,
  wid = letter,
  within = time,
  # between = group,
...
acute_summary_anova_lm_model <- lm(time_value ~ time, data = acute_summary_df)
acute_summary_anova_resid_vals <- residuals(acute_summary_anova_lm_model)

qqnorm(acute_summary_anova_resid_vals)
qqline(acute_summary_anova_resid_vals)
shapiro.test(acute_summary_anova_resid_vals)
# its like 2 points pulling it, visually I'm taking it
```

### acute with AM251 females
```{r}
# making a subset for just acute with AM251 females
AM251_summary_df <- summary_anova_df %>%
  filter(group == "AM251")
# both time (interval) and letter are as factor which is good


# HoV
leveneTest(time_value ~ time, data = AM251_summary_df)
# good
```
...
AM251_summary_anova <- ezANOVA(
  data = AM251_summary_df,
  dv = time_value,
  wid = letter,
  within = time,
  # between = group,
...
AM251_summary_anova_lm_model <- lm(time_value ~ time, data = AM251_summary_df)
AM251_summary_anova_resid_vals <- residuals(AM251_summary_anova_lm_model)

qqnorm(AM251_summary_anova_resid_vals)
qqline(AM251_summary_anova_resid_vals)
shapiro.test(AM251_summary_anova_resid_vals)
# log transform could do good here too
```

### repeated females
```{r}
# making a subset for just repeated females
repeated_summary_df <- summary_anova_df %>%
  filter(group == "repeated")
# both time (interval) and letter are as factor which is good


# HoV
leveneTest(time_value ~ time, data = repeated_summary_df)
# good
```
...
repeated_summary_anova <- ezANOVA(
  data = repeated_summary_df,
  dv = time_value,
  wid = letter,
  within = time,
  # between = group,
...
repeated_summary_anova_lm_model <- lm(time_value ~ time, data = repeated_summary_df)
repeated_summary_anova_resid_vals <- residuals(repeated_summary_anova_lm_model)

qqnorm(repeated_summary_anova_resid_vals)
qqline(repeated_summary_anova_resid_vals)
shapiro.test(repeated_summary_anova_resid_vals)
# log transform could do good here too
```