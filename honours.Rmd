...
add_new_cells(
  ...
  write_new_csv = "yes",
  new_file_name = "Data/AP_parameters.csv", # will add to this and overwrite the current version of this file
  decimal_places = 2,
  injection_start_time = 265.4,
  length_of_current_injection = 0.5
)
...
```{r}
AP_parameters <- read.csv(here("Data/AP_parameters.csv")) # puts it in the environment
```
...
raw_eEPSC_acute_plots <- plot_raw_current_data(
 data = normalized_raw_eEPSC_df %>% filter(letter %in% c("RMA1", "RMA2", "RMA3", "RMA4","RMA6", "RMA8", "RMA9", "RMA10", "SW1_MA1", "SW2_MA2", "SW3_MA3", "SW4_MA4", "SW5_MA5", "TW1_MA6", "TW2_MA7", "TW3_MA8", "RMA1A", "RMA2A", "RMA3A", "RMA4A", "RMA5A", "RMA6A")), # why are RMA3 and RMA4 not showing up (May 2025)
...
pruned_eEPSC_df <- make_pruned_EPSC_data(
  data = normalized_raw_eEPSC_df %>% filter(letter %in% c("RMA1", "RMA2", "RMA3", "RMA4", "RMA6", "RMA8", "RMA9", "RMA10", "RMA1A", "RMA2A", "RMA3A", "RMA4A", "RMA5A", "RMA6A", "LS1", "LS2", "LS3", "LS4", "LS5", "LS6", "LS7", "LS8", "LS9", "LS10", "SW1_MA1", "SW2_MA2", "SW3_MA3", "SW4_MA4", "SW5_MA5", "TW1_MA6", "TW2_MA7", "TW3_MA8")),  
  # no longer filtering bad cells RMA5 and RMA7 (2025/6/16)
  current_type = "eEPSC",
  min_time_value = 0,
  max_time_value = 30,
...
t_test_female_acute_am251 <-perform_t_tests_for_summary_plot(
  data = summary_eEPSC_df$summary_data %>% filter(letter %in% c("RMA1A", "RMA2A", "RMA3A", "RMA4A", "RMA5A", "RMA6A")), 
  test_category = 2,
  include_all_treatments = "no",
