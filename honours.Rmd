...
## ANOVA for summary ?
```{r}
# creating data frame from PRUNED data

female_anova_df <- pruned_eEPSC_df$individual_cells %>%
  filter(sex == "Female") %>% 
  mutate(group = interaction(category, treatment, drop=TRUE))
```

```{r}
# make sure it is as a factor
female_anova_df <- female_anova_df %>%
  mutate(
    letter = factor(letter),
    interval_pruned   = factor(interval_pruned),
    group  = factor(group)
  )

# there should be 4 groups: naive, acute, acute+AM251, repeated
```
...
female_anova <- aov_ez(
  id      = "letter",
  dv      = "mean_P1",
  data    = female_anova_df,
  within  = "interval_pruned",
  between = "group",
  type    = 3
) # why is TYPE 3?
...
resid_female_anova <- residuals(female_anova$lm)

qqnorm(resid_female_anova)
qqline(resid_female_anova)

shapiro.test(resid_female_anova)
```
