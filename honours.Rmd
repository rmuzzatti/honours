...
raw_eEPSC_acute_plots <- plot_raw_current_data(
 data = normalized_raw_eEPSC_df %>% filter(letter %in% c("RMA1", "RMA2", "RMA3", "RMA4","RMA6", "RMA8", "RMA9", "RMA10", "MA1", "MA2", "MA3")), # why are RMA3 and RMA4 not showing up (May 2025)
...
pruned_female_eEPSC_df <- make_pruned_EPSC_data(
  data = normalized_raw_eEPSC_df %>% filter(sex == "Female")%>% filter(letter %in% c("RMA1", "RMA2", "RMA3", "RMA4", "RMA6", "RMA8", "RMA9", "RMA10", "LS1", "LS2", "LS3", "LS4", "LS5", "LS6", "LS7", "LS8", "LS9", "LS10")),  
  # no longer filtering bad cells RMA5 and RMA7 (2025/6/16)
  current_type = "eEPSC",
  min_time_value = 0,
  max_time_value = 30,
...
pruned_eEPSC_acute_plots$RMA9
pruned_eEPSC_acute_plots$RMA10
```
...
# t-test
  stat_compare_means(
    method = "t.test", # using wilcox because did not pass assumptions
    label = "p.signif", # p.signif does the stars, p.format shows the p-value
    label.y = 150, # where I want the star, could also be "c(270, 300)" if I had multiple (when I add repeated), 
    comparisons = list(c("1", "2")),
    size = 5
) +
