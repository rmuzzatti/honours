...
```{r}
# data
acute_freq_females <- c("RMA1", "RMA2", "RMA3", "RMA4", "RMA6", "RMA8", "RMA9", "RMA10") 
acute_female_AP_count_data <- subset(AP_count_data, letter %in% acute_freq_females)
acute_female_AP_count_data


acute_female_baseline_AP_count_data <- subset(acute_female_AP_count_data, state == "Baseline")$AP_frequency
acute_female_HFS_AP_count_data <- subset(acute_female_AP_count_data, state == "HFS")$AP_frequency

acute_female_count_AP_data_diff <- acute_female_baseline_AP_count_data - acute_female_HFS_AP_count_data


# normality
qqnorm(acute_female_count_AP_data_diff) 
qqline(acute_female_count_AP_data_diff) 
shapiro.test(acute_female_count_AP_data_diff) # NOT NORMAL

# t.test(acute_female_baseline_AP_count_data, acute_female_HFS_AP_data, paired = TRUE) # idk what is happening rn and i need to run everything else

# no idea how to have this consider current injection so I am done for the day (somehow need to get it to treat it simialr to letter but also across time idek)
```
...
naive_summary_df_anova <- ezANOVA(
...
naive_summary_df_anova_lm_model <- lm(time_value ~ time, data = naive_summary_df)
naive_summary_df_anova_resid_vals <- residuals(naive_summary_df_anova_lm_model)

qqnorm(naive_summary_df_anova_resid_vals)
qqline(naive_summary_df_anova_resid_vals)
shapiro.test(naive_summary_df_anova_resid_vals)
