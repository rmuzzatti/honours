...
# group means
female_PPR_group_mean <- female_PPR_plot_df %>%
  group_by(state, category, treatment, sex, facet_group) %>%
  summarize(mean_PPR_group = mean(mean_PPR_cell, na.rm = TRUE), .groups = "drop")
   
#4 plot
female_PPR_plot <- female_PPR_plot_df %>%
  ggplot(aes(x = state, y = mean_PPR_cell)) + # removing group = letter fixed the brackets but got rid of the gray lines between cells (letters)
...
# group mean
  geom_line(data = female_PPR_group_mean,
    aes(x = state, y = mean_PPR_group, group = 1, colour = interaction(category, treatment)),
    linewidth = 1.2, inherit.aes = FALSE) +
  geom_point(data = female_PPR_group_mean,
    aes(x = state, y = mean_PPR_group, colour = interaction(category, treatment), shape = sex),
...
# add significance brackets
  geom_signif(
    ...
    map_signif_level = FALSE, # change this to TRUE if you want stars
    vjust = -0.3,
    textsize = 5,
    size = 0.5,
    margin_top = 0.1,
...
PPR female plot without repeated
```{r}
#1 facet labels
  facet_labels <- c(
    "1.Control.Female" = "Naive",
...
female_acute_PPR_plot_df <- PPR_df %>%
  filter(state %in% c("Baseline", "Post-modification")) %>% 
  filter(sex == "Female") %>% 
  filter(category != 3) %>% # excluding cat 3 (repeated), could also use category %in% c(1,2)
  mutate(
    state = ifelse(state == "Post-modification", "HFS", state)) %>% # rename to HFS
 mutate(facet_group = factor(interaction(category, treatment, sex),
                              levels = names(facet_labels))) %>%
...
# group means
female_acute_PPR_group_mean <- female_acute_PPR_plot_df %>%
  group_by(state, category, treatment, sex, facet_group) %>%
  summarize(mean_PPR_group = mean(mean_PPR_cell, na.rm = TRUE), .groups = "drop")
...
# group mean
  geom_line(data = female_acute_PPR_group_mean,
    aes(x = state, y = mean_PPR_group, group = 1, colour = interaction(category, treatment)),
    linewidth = 1.2, inherit.aes = FALSE) +
  geom_point(data = female_acute_PPR_group_mean,
    aes(x = state, y = mean_PPR_group, colour = interaction(category, treatment), shape = sex),
    size = 2.5, inherit.aes = FALSE) +  
