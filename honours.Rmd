...
```
```{r}
female_PPR_plot_df <- female_PPR_plot_df %>%
  mutate(cat_treat = case_when(
    category == 1 & treatment == "Control" ~ "1.Control",
    category == 2 & treatment == "Control" ~ "2.Control",
    category == 2 & treatment == "AM251"   ~ "2.AM251",
  ),
  cat_treat = factor(cat_treat, 
...
female_baseline_PPR_plot <- female_PPR_plot_df %>%
  ...
  geom_sina(aes(color = factor(category)),
            maxwidth = 0.2,
            shape = 16,
            size = 2,
            show.legend = FALSE) +
  ...
  scale_x_discrete(labels = c("1.Control" = "Naive\nControl",
                              "2.Control" = "Acute\nControl",
                              "2.AM251"  = "Acute\nAM251")) +
...
# how to make the label so that it just says acute once for the treatments in that category that are next to each other 
