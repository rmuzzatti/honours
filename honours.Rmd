...
pruned_male_eEPSC_df <- make_pruned_EPSC_data(
  data = normalized_raw_eEPSC_df %>% filter(sex == "Male")%>% filter(letter %in% c("SW1_MA1", "SW2_MA2", "SW3_MA3", "SW4_MA4", "SW5_MA5", "TW1_MA6", "TW2_MA7", "TW3_MA8")),  
  current_type = "eEPSC",
  min_time_value = 0,
  max_time_value = 30,
  baseline_length = 5,
...
```{r}
pruned_eEPSC_male_acute_plots <- plot_raw_current_data(
...
plot_PPR_data_multiple_treatments(
 ...
 # facet_by_treatment = "yes",
 # not spliting the treatments like it does on the website
  test_type = "t.test",
  theme_options = my_custom_theme_options,
  treatment_colour_theme = sample_treatment_names_and_colours # wont let me use my own
)
```
