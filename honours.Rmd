...
add_new_cells(
  ...
  data_type = "AP_count",
  write_new_csv = "no",
  new_file_name = "AP_count_data.csv", # will add to this and overwrite the current version of this file
  decimal_places = 2,
...
```{r}
AP_count_data <- read.csv(here("Data/AP_count_data.csv")) # puts it in the environment

AP_count_data # prints it
```
...
raw_eEPSC_acute_plots <- plot_raw_current_data(
 data = normalized_raw_eEPSC_df %>% filter(letter %in% c("RMA1", "RMA2", "RMA3", "RMA4","RMA6", "RMA8", "RMA9", "RMA10", "SW1_MA1", "SW2_MA2", "SW3_MA3", "SW4_MA4", "SW5_MA5", "TW1_MA6", "TW2_MA7", "TW3_MA8", "RMA1A", "RMA2A", "RMA3A", "RMA4A", "RMA5A", "RMA6A", "RMA7A", "RMA8A")), # why are RMA3 and RMA4 not showing up (May 2025)
...
pruned_eEPSC_df <- make_pruned_EPSC_data(
  data = normalized_raw_eEPSC_df %>% filter(letter %in% c("RMA1", "RMA2", "RMA3", "RMA4", "RMA6", "RMA8", "RMA9", "RMA10", "RMA1A", "RMA2A", "RMA3A", "RMA4A", "RMA5A", "RMA6A", "RMA7A", "RMA8A", "LS1", "LS2", "LS3", "LS4", "LS5", "LS6", "LS7", "LS8", "LS9", "LS10", "SW1_MA1", "SW2_MA2", "SW3_MA3", "SW4_MA4", "SW5_MA5", "TW1_MA6", "TW2_MA7", "TW3_MA8")),  
  # no longer filtering bad cells RMA5 and RMA7 (2025/6/16)
  current_type = "eEPSC",
  min_time_value = 0,
  max_time_value = 30,
...
pruned_naive_female_long <- pruned_eEPSC_df$individual_cells %>% filter(letter %in% c("LS1", "LS2", "LS3", "LS4", "LS5", "LS6", "LS7", "LS8", "LS9", "LS10"))
```
...
```{r}
acute_difference_evoked_df <- summary_eEPSC_df$percent_change_data %>%
...
```{r}
am251_acute_difference_evoked_df <- summary_eEPSC_df$percent_change_data %>%
  filter(letter %in% c("RMA1A", "RMA2A", "RMA3A", "RMA4A", "RMA5A", "RMA6A", "RMA7A", "RMA8A")) %>%
  mutate(
...
```{r}
male_acute_difference_evoked_df <- summary_eEPSC_df$percent_change_data %>%
  filter(letter %in% c("SW1_MA1", "SW2_MA2", "SW3_MA3", "SW4_MA4", "SW5_MA5", "TW1_MA6", "TW2_MA7", "TW3_MA8")) %>%
  mutate(
...
t_test_female_acute <-perform_t_tests_for_summary_plot(
  data = summary_eEPSC_df$summary_data %>% filter(letter %in% c("RMA1", "RMA2", "RMA3", "RMA4", "RMA6", "RMA8", "RMA9", "RMA10")), # no longer including bad cells RMA5 and RMA7 (2025/6/16)
  test_category = 2,
  include_all_treatments = "no",
  list_of_treatments = c("Control"),
  current_type = "eEPSC",
...
t_test_female_acute_am251 <-perform_t_tests_for_summary_plot(
  data = summary_eEPSC_df$summary_data %>% filter(letter %in% c("RMA1A", "RMA2A", "RMA3A", "RMA4A", "RMA5A", "RMA6A", "RMA7A", "RMA8A")), 
  test_category = 2,
  include_all_treatments = "no",
...
t_test_male_acute <-perform_t_tests_for_summary_plot(
  data = summary_eEPSC_df$summary_data %>% filter(letter %in% c("SW1_MA1", "SW2_MA2", "SW3_MA3", "SW4_MA4", "SW5_MA5", "TW1_MA6", "TW2_MA7", "TW3_MA8")), 
  test_category = 2,
  include_all_treatments = "no",
  list_of_treatments = c("Control"),
  current_type = "eEPSC",
...
```{r}
# already a sina plot, want to add violin plot aspect to this
...
```{r eval=FALSE}
# making data frame
female_baseline_amp_plot_df <- summary_eEPSC_df$percent_change_data %>%
  filter(sex == "Female") %>%
...
# the display name for Category 1 + Treatment Control is "Naive" and that is being displayed here for no reason because this should only be category 2 (acute)
```

```{r}
plot_AP_frequencies_single_treatment(
  data = sample_AP_count_data, # won't work with my data
  plot_treatment = "Control",
  plot_category = 2, # Acute with only picro
  included_sexes = "both",
  baseline_label = "Baseline",
  hormone_added = "HFS",
  significance_display_method = "stars", # could also be "p-values"
  treatment_colour_theme = my_theme_colours,
  large_axis_text = "no",
...
Creating Variance dataset for variance analysis
```{r}
