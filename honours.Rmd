...
raw_eEPSC_acute_plots <- plot_raw_current_data(
 data = normalized_raw_eEPSC_df %>% filter(sex == "Female") %>% filter(letter %in% c("RMA1", "RMA2", "RMA3", "RMA4")), # why are RMA3 and RMA4 not showing up
...
```{r}
# summarizes (averages?) data per minute

pruned_female_eEPSC_df <- make_pruned_EPSC_data(
  data = normalized_raw_eEPSC_df %>% filter(sex == "Female")%>% filter(letter %in% c("RMA1", "RMA2", "RMA3", "RMA4", "LS1", "LS2", "LS3", "LS4", "LS5", "LS6", "LS7", "LS8", "LS9", "LS10")),
  current_type = "eEPSC",
  min_time_value = 0,
  max_time_value = 30,
  baseline_length = 5,
...
acute_difference_evoked_df <- summary_female_eEPSC_df$percent_change_data %>%
  filter(letter %in% c("RMA1", "RMA2", "RMA3", "RMA4")) %>%
  mutate(
...
t_test_female_acute <-perform_t_tests_for_summary_plot(
  data = summary_female_eEPSC_df$summary_data %>% filter(letter %in% c("RMA1", "RMA2", "RMA3", "RMA4")), # don't actual need to filter these letters
  test_category = 2,
  include_all_treatments = "no",
  list_of_treatments = c("Control"),
  current_type = "eEPSC",
