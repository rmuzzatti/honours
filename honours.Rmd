...
female_PPR_plot <- female_PPR_plot_df %>%
  ggplot(aes(x = state, y = mean_PPR_cell, group = letter)) +

# individual  cells  
  geom_line(alpha = 0.5, colour = "lightgrey") + 
...
# add t-test p-values
  stat_compare_means(
    data = female_PPR_plot_df,
    comparisons = list(c("Baseline", "HFS")),
    aes(group = state),
    method = "t.test",
    label.x = 1.25, # centers p-value (or at least the start of it...)
    label = "p.format", # or p.value
    bracket.size = 0.5,
    tip.length = 0.02,
    size = 3,
    group.by = "facet_group",
    label.y = max(female_PPR_plot_df$mean_PPR_cell, na.rm = TRUE) * 1.05 # smaller brings it down, bigger brings it up
  ) +
